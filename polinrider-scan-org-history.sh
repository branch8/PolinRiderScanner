...
[SUSPICIOUS] Malware runtime require hook: global['r']=require|$COMMON_GLOBAL_R"
    ...
    while IFS= read -r sig_entry; do
        ...
        while IFS= read -r hit_line; do
            ...
            case "$filepath" in
                *.woff2|*.woff)
                    # Determine which marker matched to build the WOFF2 description
                    case "$sig_label" in
                        *'[V1]'*)
                        ...
                        *'[SUSPICIOUS]'*)
                            ...
                            ;;
                        *)
                            printf 'FINDING\t%s\t%s\t[WOFF2] Fake font file — %s\n' "$branch" "$filepath" "$sig_label" >> "$results_file"
                            ;;
                    esac
                    ;;
                *)
...
SIGSEOF
    ...
    # --- Pass 7b: .gitignore propagation script hiding check ---
    local gitignore_out
    ...
    # --- Pass 7c: Fabricated fonts directory README ---
    ...
    # --- Pass 7d: Malicious PyPI packages (graphalgo cluster — Lazarus 2025-05+) ---
    _ep "PyPI packages..."
    old_ifs="$IFS"
    IFS=' '
    for mal_pypi in $MALICIOUS_PYPI_PKGS; do
        ...
        pypi_out=$(mktemp)
        # shellcheck disable=SC2086
        git -C "$bare_dir" grep -lF "$mal_pypi" $all_refs -- \
        ...
        if [ -s "$pypi_out" ]; then
            while IFS= read -r hit_line; do
                ...
                # Confirm — substring match isn't enough; verify it's a dependency declaration
    ...
    # --- Pass 7e: GitHub Actions workflow analysis ---
    local gha_out
    gha_out=$(mktemp)
    # shellcheck disable=SC2086
    ...
    # --- Pass 7f: build/CI artifact scan (Dockerfile / Makefile / monorepo / npm scripts) ---
    local build_out
    build_out=$(mktemp)
    # shellcheck disable=SC2086
    git -c grep.threads=4 -C "$bare_dir" grep -lE \
    ...
    # --- Pass 7g: .gitmodules URL allowlist ---
    local gm_out
    gm_out=$(mktemp)
    # shellcheck disable=SC2086
    ...
    # --- Pass 7i: Suspicious font-folder fingerprints (file-name level) ---
    local font_out
    font_out=$(mktemp)
    # shellcheck disable=SC2086
